...
@component_registry.add
class PeriodicTaxBracketMultiOrchestrator(BaseComponent):
    name = "PeriodicTaxBracketMultiOrchestrator"
    component_type = "PeriodicTax"
    required_entities = ["Coin"]
    ...
    def __init__(
        ...
        **base_component_kwargs
        ...
        self.workers = {
            country:PeriodicBracketTaxMultiWorker(country, *base_component_args, **base_component_kwargs)
            for country in range(self.world.number_of_states)
        }

    # Required methods for implementing components
    # --------------------------------------------

    def get_n_actions(self, agent_cls_name, country=0):
        """
        See base_component.py for detailed description.
        ...
        """
        # Only the planner takes actions through this component.
        if agent_cls_name == "MultiPlanner":
            worker = self.workers[country]
            if worker.tax_model == "model_wrapper" and not worker.disable_taxes:
                # For every bracket, the planner can select one of the discretized
                # tax rates.
                return [
                    ("TaxIndexBracket_{:03d}".format(int(r)), worker.n_disc_rates)
                    for r in worker.bracket_cutoffs
                ]
    ...
    def generate_observations(self):
        ...
        for _, worker in self.workers.items():
            ...
            obs.update(worker.generate_observations())
        return obs

    def generate_masks_(self, completions=0):
        """
        See base_component.py for detailed description.
    ...
    def generate_masks(self, completions=0):
        ...
        for worker in self.workers.values():
            ...
            masks.update(worker.generate_masks(completions=completions))
...
class PeriodicBracketTaxMultiWorker(BaseComponent):
    ...
    name = "PeriodicBracketTaxMultiWorker"
    component_type = "PeriodicTax"
    required_entities = ["Coin"]
    agent_subclasses = ["Citizen", "MultiPlanner"]
    ...
    def __init__(
        ...
        **base_component_kwargs
        ...
        if self.tax_model == "model_wrapper" and not self.disable_taxes:
            planner_action_tuples = self.get_n_actions("MultiPlanner")
            self._planner_tax_val_dict = {
                k: self.disc_rates for k, v in planner_action_tuples
            }
        else:
    ...
    def set_new_period_rates_model(self):
        ...
        # AI version
        for i, bracket in enumerate(self.bracket_cutoffs):
            planner = self.world.get_planner_by_country(self.country)
            planner_action = self.world.planner.get_component_action(
                'PeriodicTaxBracketMultiOrchestrator', "TaxIndexBracket_{:03d}".format(int(bracket))
            )
            if planner_action == 0:
                pass
    ...
    def enact_taxes(self):
        ...
        self.last_marginal_rate = []
        for agent, last_coin in zip(self.world.agents, self.last_coin):
            if agent.state['nation'] == self.country:
                ...
                #agent not currently in country do nothing
                self.last_income.append(float(0))
                self.last_marginal_rate.append(float(0))
                self.last_effective_tax_rate.append(0)

        self.total_collected_taxes += float(net_tax_revenue)

        lump_sum = net_tax_revenue / len([agent for agent in self.world.agents
                                            if agent.state["nation"] == self.country])
        for agent in self.world.agents:
            if agent.state["nation"] == self.country:
                agent.state["inventory"]["Coin"] += lump_sum
        ...
        if self.tax_model == "model_wrapper":
            ...
            if self.tax_annealing_schedule is None:
                ...
                if self.tax_cycle_pos != 1 or self.disable_taxes:
                    # Apply zero masks for any timestep where taxes
                    # are not going to be updated.
                    masks[f'p_{self.country}'] = self._planner_masks["zeros"]
                else:
                    masks[f'p_{self.country}'] = self._planner_masks["new_taxes"]
    ...
    def additional_reset_steps(self):
        ...
        self.last_income = [0 for _ in range(self.world.n_agents)]
        self.last_marginal_rate = [0 for _ in range(self.world.n_agents)]
        self.last_effective_tax_rate = [0 for _ in range(self.world.n_agents)]
    ...
    def get_metrics(self):
        ...
        if not self.disable_taxes:
            ...
            country_specific_mask = np.array([int(agent.state["nation"] != self.country) 
                        for agent in self.world.agents])
            masked_agents = ma.masked_array(agent_coin_endows, country_specific_mask)

            #only take into account residents
            idx_poor = masked_agents.argmin()
            idx_rich = masked_agents.argmax()
            tax_days = self.taxes[(self.period - 1) :: self.period]
            for i, tag in zip([idx_poor, idx_rich], ["poorest", "richest"]):
